...
/**
 * Bridge-issued opaque session identifier. One {@code SessionId} is minted at WebSocket handshake
 * (post-Auth, when the {@code sub} claim is known) and dies when the WebSocket closes — never
 * reused.
 *
 * <p><b>Purpose.</b> Stable, hashable key for the per-session quote-correlation indexes in {@link
 * SessionQuoteIndex} and for the audit-log {@code session_terminated} fan-out path. Wraps a {@code
 * String} so map lookups go through the JDK's {@code String} hash + equals (mature, well- tested)
 * without leaking the implementation choice to callers.
 *
 * <p><b>Threading.</b> Immutable; safe to share across threads. The Netty event-loop ownership
 * invariant of {@link SessionQuoteIndex} (single-thread mutation) is enforced on the index, not on
 * this value type.
 *
