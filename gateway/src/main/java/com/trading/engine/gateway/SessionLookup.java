...
/**
 * Correlates a request identifier (ClOrdID, QuoteReqID, or any correlation key) to the
 * gateway-assigned FIX session key that originated the request. Used by the {@link
 * ClusterEgressListener} to route cluster responses back to the correct Artio session.
 *
...
public interface SessionLookup {
  ...
  /**
   * Find the gateway session key for the request with the given correlation identifier.
   *
   * @param correlationId null-padding-trimmed correlation ID bytes (e.g., ClOrdID or QuoteReqID)
   * @param offset start offset within {@code correlationId}
   ...
   */
  long findByCorrelationId(byte[] correlationId, int offset, int length);
}
