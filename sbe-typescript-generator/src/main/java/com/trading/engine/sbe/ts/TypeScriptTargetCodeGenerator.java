 ...
 * is NOT a {@link java.util.ServiceLoader}-based SPI. However, {@code
 * TargetCodeGeneratorLoader.get(name)} falls back to {@link Class#forName(String)}{@code
 * .getConstructor().newInstance()} for any fully-qualified class name passed via {@code
 * -Dsbe.target.language=...}. This class is the FQCN that the Gradle task {@code
 * :sbe-typescript-generator:generateTsCodecs} hands to {@code SbeTool}; the public no-arg
 * constructor is required by that reflective load path.
 *
 ...
 *       rejected because it forks an upstream library we want to track at version 1.37.1, and the
 *       FQCN-fallback path is the documented escape hatch for exactly this case.
 *   <li><b>Post-process the C/Java generator's output</b> — rejected because SBE's generated C and
 *       Java codecs are imperative class structures; deriving a {@code DataView}-flyweight
...
public final class TypeScriptTargetCodeGenerator implements TargetCodeGenerator {
  ...
  /**
   * Build the orchestrating code generator that walks {@code ir} and writes per-message TypeScript
   * files into {@code outputDir}.
   *
   * <p>Build-time invocation only. {@code SbeTool} calls this exactly once per build; the returned
   * object is then used exclusively from the calling thread.
   *
   * <p>The supertype's {@link TargetCodeGenerator#newInstance(Ir, String)} declares no checked
   * exceptions, so this override declares none either; any I/O failure that occurs while walking
   * the IR or writing files happens later, inside {@link CodeGenerator#generate()}, which the SPI
   * explicitly permits to throw {@link java.io.IOException}.
   *
   * @param ir the parsed SBE intermediate representation; must not be {@code null}
   * @param outputDir absolute path to the directory the generator must populate; the directory may
   *     or may not exist on entry — the generator must create it if necessary. Must not be {@code
   *     null} or blank.
